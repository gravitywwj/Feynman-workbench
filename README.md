...
- 复习模块把“计划复习”和“突击检查”分开：默认在学习后第 1、3、7 天安排主动回忆，并依照“没记住 / 很吃力 / 记得住 / 很熟”调整后续间隔；可选择费曼教练梳理，或由直截了当的突击教练检查。两种反馈均只依据 Wiki 资料与学习记录生成，并保存在本地。
- 可在“学习记录”继续补充待澄清点、查看历史会话、导出本地学习数据；Wiki 页面移动后可手动重新关联记录。
- 配置 `DEEPSEEK_API_KEY` 时使用模型依据参考资料诊断；未配置时仅提供可追溯的“表达结构提示”，不会宣称核验了知识事实。用户可标记误判，反馈会随本地学习数据导出。
- 今日学习时长来自已完成的回顾会话；图谱、首页与下一概念会显示明确的推荐理由，而不是将标题排序包装成推荐。
